...
export const projectKeys = {
	...
	joinRequests: (id: string) =>
		[...projectKeys.detail(id), "join-requests"] as const,
...
export const useCreateJoinRequest = () => {
	...
	return useMutation({
		mutationFn: async (params: { projectId: string; message?: string }) => {
			const response = await apiClient.post<ApiResponse<any>>(
...
export const useDeleteRelease = () => {
	...
	return useMutation({
		mutationFn: async (params: { projectId: string; releaseId: string }) => {
			const response = await apiClient.delete<ApiResponse<null>>(
				`/api/v1/projects/${params.projectId}/releases/${params.releaseId}`,
			);
