...
export const seedFeedback = async (users: IUser[], projects: IProject[]) => {
	const feedbackItems = [
		{
			...
			description: 'Whenever I try to upload photos of my workouts, the app crashes immediately. This happens every time on my device.',
			type: 'bug',
		...
		},
		{
			...
			description: 'It would be great to have a dark mode option for late-night workout logging',
			type: 'feature',
			category: 'ui-ux',
			status: 'open',
			priority: 'medium',
		...
		},
		{
			...
			description: 'Some tasks are not appearing in the calendar view even though they have due dates set',
			type: 'bug',
			category: 'functionality',
			status: 'in-progress',
			priority: 'high',
		...
		},
		{
			...
			description: 'The AI task prioritization is incredibly accurate and has really helped me stay organized',
			type: 'improvement',
			category: 'feedback',
			status: 'resolved',
			priority: 'low',
		...
		},
		{
			...
			description: 'Uploading high-res photos takes several minutes. Need better compression or background upload',
			type: 'bug',
	...
	for (const feedbackData of feedbackItems) {
		...
		// Add some comments
		const commenters = users.filter(u => 
			u._id.toString() !== feedbackData.userId?.toString()
		).slice(0, 2)
