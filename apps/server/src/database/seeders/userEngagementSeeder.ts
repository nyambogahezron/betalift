...
export const seedUserEngagement = async (users: IUser[]) => {
	...
	for (const user of users) {
		...
		if (engagement) {
			...
			engagement.testingExperience = (["beginner", "intermediate", "expert"] as const)[Math.floor(Math.random() * 3)] as "beginner" | "intermediate" | "expert";
			const toStatus = (["available", "busy", "away"] as const)[Math.floor(Math.random() * 3)] as "available" | "busy" | "away";
			engagement.availability = {
				status: toStatus,
				hoursPerWeek: Math.floor(Math.random() * 20) + 5,
				timezone: "UTC",
				preferredContactMethod: (["in-app", "email"] as const)[Math.floor(Math.random() * 2)] as "in-app" | "email",
			};
