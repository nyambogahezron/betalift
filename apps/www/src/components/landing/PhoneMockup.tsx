...
import { Wifi, Signal, Battery, ChevronLeft, Share, Sparkles } from "lucide-react";
...
const posts = [
  {
    ...
    content: "iOS and Android are two major mobile operating system are using all around the world.",
    author: "Harry",
    sources: ["K", "N", "W"],
    likes: 12,
    comments: 9,
          ...
          <div className="bg-card rounded-[2.5rem] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-foreground">Betalift</h3>
                  <p className="text-[10px] text-muted-foreground">Clear Facts, Civil Voices</p>
                </div>
              </div>
              ...
              {posts.map((post, i) => (
                <div key={i} className="bg-secondary/50 rounded-xl p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[9px] font-medium ${post.statusColor}`}>
                      {post.status}
                    </span>
                  </div>
                  <p className="text-xs text-foreground mb-2 line-clamp-2">{post.content}</p>
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-[9px] text-muted-foreground ml-1">Sources</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      <div className="w-5 h-5 rounded-full bg-gradient-to-br from-primary to-accent" />
                      <span className="text-[10px] font-medium text-foreground">{post.author}</span>
                    </div>
